...
static void cpufreq_darkness_timer_resched(
...
static unsigned int freq_to_targetload(
	struct cpufreq_darkness_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer)) {
		pcpu->last_evaluated_jiffy = get_jiffies_64();
		cpufreq_darkness_timer_resched(pcpu);
	}
...
static void cpufreq_darkness_idle_end(void)
{
	struct cpufreq_darkness_cpuinfo *pcpu =
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_darkness_timer(smp_processor_id());
	}
...
static int cpufreq_darkness_speedchange_task(void *data)
{
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			struct cpufreq_darkness_cpuinfo *pjcpu;
			u64 hvt = ~0ULL;
...
static struct notifier_block cpufreq_darkness_idle_nb = {
	.notifier_call = cpufreq_darkness_idle_notifier,
};
	...
	unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			cpufreq_darkness_timer_start(tunables, j);
...
static int __init cpufreq_darkness_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_darkness_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_darkness_nop_timer;
